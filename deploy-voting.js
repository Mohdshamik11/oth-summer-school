...
const DEFAULT_TOPICS = ["Masala Dosai", "Briyani", "Pani Puri"];
...
async function main() {
  ...
  // The admin is a separate account from the owner: it is the only role that
  // can change the blocklist.
  let adminAddress;
  let adminLabel;
  ...
  const response = await new ContractCreateFlow()
    ...
    .setContractMemo("OTH summer school - voting")
